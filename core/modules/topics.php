...
class topics {
    ...
    public function run() {
        global $auth, $user, $comments, $cache;
            ...
            if (empty($error_msg)) {
                ...
                if (empty($error_msg) && !$is_auth['auth_mod']) {
                    $sql = 'SELECT MAX(post_time) AS last_post_time
                        FROM _forum_posts
                        WHERE poster_id = ?';
                    if ((int) $last_post_time = sql_field(sql_filter($sql, $user->d('user_id')), 'last_post_time')) {
                        if ($last_post_time && ($current_time - $last_post_time) < (int) config('flood_interval')) {
        ...
        $this->title = $forum_row['forum_name'];

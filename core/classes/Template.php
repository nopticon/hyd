...
class Template {
    ...
    // $this->vars[varname] == value  or  $this->tpldata['.'][0][varname] == value
    // array "vars" is added for easier access to data
    public $tpldata = array('.' => array(0 => array()));
    public $vars;
    ...
    public function pparse($handle) {
        ...
        if (empty($this->compiled_code[$handle])) {
            ...
            if (!empty($this->files_cache2[$handle]) && empty($this->files_cache[$handle]) && !$force_recompile) {
                $this->compiled_code[$handle] = $this->compile2(
        ...
        for ($i = 0; $i < $line_count; $i++) {
            ...
            */
            if ($keyword_type == XS_TAG_BEGIN) {
                ...
                if ($block_nesting_level < 2) {
                    ...
                    $line = '<'."?php ";
                    $line .= '$'. $var. '_count = ';
                    $line .= 'isset(' . $td . '[\''. $var. '.\']) ?  sizeof(' . $td . '[\''. $var. '.\']) : 0;';
                    $line .= ' for ($'. $var. '_i = 0; $'. $var. '_i < $'. $var. '_count; $'. $var. '_i++)';
                    $line .= '{';
                    $line .= ' $'. $var. '_item = &' . $td . '[\''. $var. '.\'][$'. $var. '_i];';
            ...
            */
            if ($keyword_type == XS_TAG_INCLUDE) {
                ...
                $this->include_count++;
                $line = '<'.'?php $this->set_filename(\'xs_include_' . $filehash . '\', \'' . $params_str .'\', true);';
    ...
    */
    public function _compile_text($code) {
        ...
        if ($this->replace_vars) {
            $code_vars = 'echo isset($this->vars[\'\1\']) ? $this->vars[\'\1\'] : $this->lang(\'\1\'); ?'.'>';

            $code_def = '$this->tpldata[\'DEFINE\'][\'.\'][\'\\1\']';
            $code_def = '<'.'?php echo isset(' . $code_def . ') ? ' . $code_def . ' : \'\'; ?'.'>';

            $code = preg_replace('#\{([a-z0-9\-_]*?)\}#is', '<'.'?php ' . $code_vars, $code);
            $code = preg_replace('#\{\$([a-z0-9\-_]*?)\}#is', $code_def, $code);
        }
        return $code;
    }
        ...
        for ($i = 0; $i < count($tokens); $i++) {
            ...
            switch ($token) {
                ...
                case 'is':
                    ...
                    // Merge with default case
                default:
                    if (preg_match('#^(([a-z0-9\-_]+?\.)+?)?(\$)?([A-Z]+[A-Z0-9\-_]+)$#s', $token, $varrefs)) {
                        if (!empty($varrefs[1])) {
                            $token  = $this->generate_block_data_ref(substr($varrefs[1], 0, -1), true, $varrefs[3]);
                            $token .= '[\'' . $varrefs[4] . '\']';
                        } else {
                            $token = $varrefs[3] ? '$this->tpldata[\'DEFINE\'][\'.\'][\'' . $varrefs[4] . '\']' : 0;
                            $token = $token ?: '$this->vars[\'' . $varrefs[4] . '\']';
                        }
                    }
                    break;
            }
    ...
    public function compile_tag_undefine($tag_args) {
        ...
        $result = $result ?: '$this->tpldata[\'DEFINE\'][\'.\'][\'' . $match[3] . '\']';
    ...
    */
    public function flush_block_vars($blockname) {
        ...
        $current_iteration = sizeof($this->tpldata[$blockname . '.']) - 1;
...
if (!function_exists('xs_switch')) {
    function xs_switch($tpl, $name) {
        return (isset($tpl->tpldata[$name.'.']) && count($tpl->tpldata[$name.'.']) > 0);
    }
}

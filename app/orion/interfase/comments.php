...
class comments {
	...
	public function emoticons() {
		...
		foreach ($emoticons as $smile_url => $row) {
			_style('smilies_row', array(
				'CODE' => $row->code,
				'IMAGE' => assets_url() . '/emoticon/' . $row->smile_url,
				'DESC' => $row->emoticon)
			);
		}
	}
	...
	private function parse_smilies() {
		...
		if (!isset($this->options['smilies'])) {
			...
			foreach ($smilies as $row) {
				...
				$this->options['smilies']['repl'][] = ' <img src="' . assets_url() . '/emoticon/' . $row->smile_url . '" alt="' . $row->emoticon . '" />';
			}
		}
	...
	private function parse_avatars() {
		...
		if (preg_match_all('#\:i([0-9a-zA-Z\_\- ]+)\:#si', $this->message, $match)) {
			...
			$assets_url = assets_url() . 'avatars/';
			$assets_path = assets_path() . 'avatars/';

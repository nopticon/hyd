...
function monetize() {
	...
	foreach ($monetize as $row) {
		...
		_style('monetize.' . $row->monetize_position . '.row', array(
			...
			'IMAGE' => assets_url() . 'base/' . $row->monetize_image,
...
function generate_path($name) {
	...
	return $config->project_path . $config->{$name . '_path'};
}

function assets_path() {
	return generate_path('assets');
}

function artists_path() {
	return generate_path('artists');
}

function events_path() {
	return generate_path('events');
}

function news_path() {
	return generate_path('news');
}

function cache_path() {
	return generate_path('cache');
}

function generate_url($name) {
	global $config;

	return s_link() . $config->{$name . '_url'};
}

function assets_url() {
	return generate_url('assets');
}

function artists_url() {
	return generate_url('artists');
}

function events_url() {
	return generate_url('events');
}

function news_url() {
	return generate_url('news');
}
...
function r($regex, $delimiter = '/', $options = 'i') {
	return $delimiter . $regex . $delimiter . $options;
}
...
function artist_root($alias, $check = false) {
	...
	$response = artists_path() . artist_build($alias);
...
function artist_path($alias, $id, $build = true, $check = false) {
	...
	if ($build) {
		$response = artists_path() . artist_build($response) . '/';
	}

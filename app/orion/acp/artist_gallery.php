...
class __artist_gallery extends mac {
	...
	public function _home() {
		...
		foreach ($result as $i => $row) {
			...
			_style('gallery.row', array(
				...
				'IMAGE' => artists_url() . $this->object->ub . '/thumbnails/' . $row->image . '.jpg',
				'RIMAGE' => get_a_imagepath(artists_url() . $this->object->ub, $row->image . '.jpg', w('x1 gallery')),
				'WIDTH' => $row->width,
				'HEIGHT' => $row->height,
	...
	*/
	private function upload() {
		...
		$filepath = artists_path() . $this->object->ub . '/';
	...
	*/
	private function remove() {
		...
		if (count($s_images)) {
			$common_path = artists_path() . $this->object->ub . '/';
			$path = array(
				$common_path . 'x1/',
				$common_path . 'gallery/',

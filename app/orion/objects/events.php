...
class events {
	...
	public function future() {
		...
		foreach ($result as $i => $row) {
			...
			$event_mini = events_path() . 'mini/' . $row->id . '.jpg';
			...
			_style('events_next.row', array(
				...
				'IMAGE' => events_url() . $event_image . $row->id . '.jpg?u=' . $row->event_update)
			);
		}
	...
	public function past($start = 0) {
		...
		foreach ($result as $i => $row) {
			...
			_style('events_past.row', array(
				'URL' => s_link('events', $row->event_alias),
				'TITLE' => $row->title,
				'IMAGE' => events_url() . 'gallery/' . $row->id . '/thumbnails/' . $row2->image . '.jpg?u=' . $row->event_update)
			);
		}
			...
			default:
				...
				*/
				if ($user->is('colab') && !$this->v('images') && _button('create_thumbnail')) {
					$location_large = events_path() . 'future/' . $this->v('id') . '.jpg';
					$location_mini = events_path() . 'mini/' . $this->v('id') . '.jpg';
				...
				*/
				...
				if ($mode == 'view') {
					...
					_style('selected', array(
						'IMAGE' => events_url() . 'gallery/' . $this->v('id') . '/' . $imagedata->image . '.jpg',
				...
				if ($this->v('images')) {
					...
					foreach ($result as $row) {
						_style('thumbnails.item', array(
							...
							'IMAGE' => events_url() . 'gallery/' . $this->v('id') . '/thumbnails/' . $row->image . '.jpg',
							'RIMAGE' => events_url() . 'gallery/' . $this->v('id') . '/' . $row->image . '.jpg',
					...
					_style('event_flyer', array(
						'IMAGE_SRC' => events_url() . 'future/' . $this->v('id') . '.jpg?u=' . $this->v('event_update'))
					);

					$location_mini = events_path() . 'mini/' . $this->v('id') . '.jpg';

					if ($user->is('colab') && !$this->v('images') && !@file_exists($location_mini)) {
						$large_filepath = events_path() . 'future/' . $this->v('id') . '.jpg';
	...
	public function all() {
		...
		if ($total_gallery = count($this->data['is_gallery'])) {
			...
			foreach ($gallery as $item) {
				_style('gallery.item', array(
					...
					'TITLE' => $item->title,
					'IMAGE' => events_url() . 'gallery/' . $item->id . '/thumbnails/' . $random_images[$item->id] . '.jpg',
		...
		foreach ($this->data as $is_date => $data) {
			...
			foreach ($data as $item) {
				$event_mini = events_path() . 'mini/' . $item->id . '.jpg';
				$event_image = events_url();

	...
	public function _main() {
		...
		if ($this->data->images) {
			...
			_style('ub_image', array(
				'IMAGE' => artists_url() . $this->data->ub . '/gallery/' . $row->image . '.jpg')
			);
	...
	public function _bio() {
		...
		if ($this->data->featured_image) {
			_style('featured_image', array(
				'IMAGE' => artists_url() . $this->data->ub . '/gallery/' . $this->data->featured_image . '.jpg',
			...
			default:
				if ($mode == 'view') {
					...
					_style('selected', array(
						'IMAGE' => artists_url() . $this->data->ub . '/gallery/' . $imagedata->image . '.jpg',
				...
				foreach ($result as $i => $row) {
					...
					_style('thumbnails.row', array(
						'URL' => s_link('a', $this->data->subdomain, 'gallery', $row->image, 'view'),
						'IMAGE' => artists_url() . $this->data->ub . '/thumbnails/' . $row->image . '.jpg',
						'RIMAGE' => get_a_imagepath(artists_path(), artists_url(), $this->data->ub, $row->image . '.jpg', w('x1 gallery')),
						'WIDTH' => $row->width,
						'HEIGHT' => $row->height,
		...
		foreach ($a_records as $row) {
			...
			if (isset($ai_records[$row->ub])) {
				...
				_style('a_records.item.image', array(
					'IMAGE' => artists_url() . $row->ub . '/thumbnails/' . $ai_records[$row->ub][$ai_select] . '.jpg')
				);
			}
		}
	}
			...
			if (count($a_random)) {
				...
				if (isset($a_random[$selected_image])) {
					_style('a_stats.gallery', array(
						'IMAGE' => artists_url() . $all_data['datetime']->ub . '/thumbnails/' . $a_random[$selected_image] . '.jpg',
	...
	public function thumbnails() {
		...
		foreach ($artists as $i => $row) {
			...
			_style('thumbnails.item', array(
				'NAME' => $row->name,
				'IMAGE' => artists_url() . $row->ub . '/thumbnails/' . $row->image . '.jpg',
				'URL' => s_link('a', $row->subdomain),
					...
					foreach ($events as $is_date => $data) {
						...
						foreach ($data as $item) {
							_style('events_future.set.row', array(
								...
								'THUMBNAIL' => events_url() . 'future/thumbnails/' . $item->id . '.jpg',
								'SRC' => events_url() . 'future/' . $item->id . '.jpg')
							);
						}
					}
				}

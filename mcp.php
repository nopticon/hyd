<?php
// -------------------------------------------------------------
// $Id: control.php,v 1.0 2006/09/06 23:43:00 Psychopsia Exp $
//
...
switch ($mode)
{
	case 'ucm':
		...
		$username_base = '';
		if ($updata = $db->sql_fetchrow($result))
		{
			$username_base = $updata['username_base'];
		}
		$db->sql_freeresult($result);
	...
	case 'merge':
		if ($submit)
		{
			...
			$sql = 'UPDATE _forum_posts
				SET forum_id = ' . (int) $to_forum_id . ', topic_id = ' . (int) $to_topic . '
				WHERE topic_id = ' . (int) $from_topic;
			$db->sql_query($sql);
...
<input type="submit" name="submit" value="Enviar" />
	...
	case 'edit':
		...
		if (!$topic = $db->sql_fetchrow($result))
		{
			fatal_error();
		}
		$db->sql_freeresult($result);
		...
		if ($submit)
		{
			...
			{
				$sql = "UPDATE _forum_topics
					SET topic_title = '" . $db->sql_escape($topic_title) . "'
					WHERE topic_id = " . (int) $topic['topic_id'];
				$db->sql_query($sql);
				
				$sql = 'SELECT id
					FROM _events
					WHERE event_topic = ' . (int) $topic['topic_id'];
				$result_event = $db->sql_query($sql);
				
				if ($row_event = $db->sql_fetchrow($result_event))
				{
					$sql = "UPDATE _events SET title = '" . $db->sql_escape($topic_title) . "'
						WHERE id = " . (int) $row_event['id'];
					$db->sql_query($sql);
				}
				$db->sql_freeresult($result_event);
			}
			...
			if ($post_message != $row['post_text'])
			{
				...
				$sql = 'INSERT INTO _forum_posts_rev' . $db->sql_build_array('INSERT', $rev);
				$db->sql_query($sql);
			}
	...
	case 'topic':
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$sql = 'UPDATE _members
				SET user_posts = user_posts - ' . (int) $row['posts'] . '
